...
async def start_cmd(message: types.Message, session: AsyncSession):
    ...
    if user is None:
        text_mgs += ('Не смог найти Вас в базе.\n'
                    'Давайте заполним данные, чтобы обратиться к Администратору чата за авторизацией.')

        btns = {"Заполнить данные": "insert_data_user",
                "Нет, спасибо": "cancel_data"}
    else:
        text_mgs += 'О! Вижу ты тут не в первой. Будешь уточнять свои данные?'

        btns = {"Уточнить данные": "edit_data_user",
                "Нет, спасибо": "cancel_data"}
...
async def add_user(callback_query: types.CallbackQuery, state: FSMContext):
    await callback_query.message.answer('Введите Ваше имя', reply_markup=types.ReplyKeyboardRemove())
    await state.set_state(AddUser.name)
    await callback_query.answer()


@user_private_router.callback_query(F.data == "edit_data_user")
async def add_user(callback_query: types.CallbackQuery, state: FSMContext, session: AsyncSession):
    await callback_query.message.answer('Хорошо, давайте отредактируем Ваши данные. '
                                        '\nДля сохранения текущего занчения введите "." (точку)',
                                        reply_markup=types.ReplyKeyboardRemove())
    user = await orm_get_user(session=session, user_id=callback_query.from_user.id)
...
async def add_name(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    await message.answer('Введите номер квартиры', reply_markup=types.ReplyKeyboardRemove())
...
async def add_apartment(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    if message.text == '.':
        ...
        if not message.text.isdigit() or not (0 < int(message.text) <= APARTMENTCOUNT):
            await message.answer('Не корректный ввод номера квартиры. Такой квартиры в доме нет.')
            return
        await state.update_data(apartment=message.text)
    if user:
        await message.answer(f'Ваш номер телефона {user.phone}')
    await message.answer('Введите телефон в формате +7ХХХХХХХХХХ', reply_markup=types.ReplyKeyboardRemove())
...
async def add_phone(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    if message.text == '.':
        ...
        if not phone.startswith('+7') or len(phone)!=12 or not phone[1:].isdigit():
            await message.answer('Не корректный формат телефона. Введите телефон в формате +7ХХХХХХХХХХ')
            return
    ...
    await message.answer('Спасибо, данные добавлены. Обратитесь к Администратору для подтверждения данных',
                         reply_markup=types.ReplyKeyboardRemove())
    await state.clear()
...
async def cancel_cmd(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback_query.message.answer('Был рад пообщаться user_private_router.')


@user_private_router.callback_query()
async def debug_all_callbacks(callback_query: types.CallbackQuery):
    print(f"Получен callback_data в user_private_router: '{callback_query.data}'")
