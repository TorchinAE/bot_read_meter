...
async def add_user(callback_query: types.CallbackQuery,
    ...
    await callback_query.message.answer('Хорошо, давайте отредактируем '
                                        'Ваши данные. '
...
async def add_apartment(message: types.Message, state: FSMContext,
    ...
    if message.text == '.' and user:
        ...
        (0 < int(message.text) <= APARTMENTCOUNT)):
            await message.answer('Не корректный ввод номера квартиры. '
    ...
    await message.answer('Введите телефон в формате +7ХХХХХХХХХХ',
                         reply_markup=types.ReplyKeyboardRemove())
...
async def add_phone(message: types.Message,
    ...
    if message.text == '.' and user:
        ...
        if await orm_get_phone(session=session, phone=phone):
            await message.answer(f'Номер телефона {phone} уже занят, '
                                 'введите другой')
            return
...
async def debug_all_callbacks(callback_query: types.CallbackQuery):
    print(f"Получен callback_data в user_private_router:'"
